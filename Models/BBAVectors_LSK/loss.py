...
class FocalLoss(nn.Module):
    ...
    def forward(self, pred, gt):
        ...
        neg_loss = torch.log(torch.clamp(1 - pred, min=1e-5)) * torch.pow(pred, 2) * neg_weights * neg_inds
...
class LossAll_nan(torch.nn.Module):
    def __init__(self):
        super(LossAll_nan, self).__init__()
        self.L_hm = FocalLoss()
    ...
    def forward(self, pr_decs, gt_batch):
        ...
        return loss, {"total_loss": loss.item(), "hm_loss": hm_loss.item()}
