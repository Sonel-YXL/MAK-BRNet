...
from Models.CFLBBA.convnext import convnext_tiny,convnext_small,convnext_base,convnext_large
...
class Cfpn(nn.Module):
    def __init__(self, backbone_name, backbone_args, fpn_name, fpn_args):
        ...
        self.neck = build_neck(fpn_name, **fpn_args)
        self.gradients = None
...
class CFLBBA(nn.Module):
    def __init__(self, backbone_name, backbone_args, fpn_name, fpn_args, heads, down_ratio, final_kernel, head_conv):
        super(CFLBBA, self).__init__()
        self.down_ratio = down_ratio
        channels = [3, 64, 256, 512, 1024, 2048]
        assert down_ratio in [2, 4, 8, 16]
        self.l1 = int(np.log2(down_ratio))

        self.base_network = Cfpn(backbone_name, backbone_args, fpn_name, fpn_args)
...
if __name__ == '__main__':
    ...
    arch_model = CFLBBA(**model_cfg['args'])
