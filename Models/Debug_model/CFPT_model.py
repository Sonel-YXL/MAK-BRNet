...
class CrossLayerSpatialAttention(nn.Module):
    ...
    def forward(self, x_list, extra=None):
        ...
        for i, x in enumerate(x_list):
            ...
            pad_i = (
            math.ceil((stride_i * (WmH - 1.) - H + ws_i) / 2.), math.ceil((stride_i * (WmW - 1.) - W + ws_i) / 2.))
...
class CFPT(nn.Module):
    def __init__(self, in_channels=(256, 512, 1024, 2048), out_channels=256, alpha=1, beta=1, num_heads=4, mlp_ratio=2,
                 reduction=4, init_cfg=None):
...
if __name__ == '__main__':
    w, h = 608, 608
    model1 = CFPT(in_channels=[64, 128, 256, 512])
    print(h / 4, h / 8, h / 16, h / 32)
    print(w / 4, w / 8, w / 16, w / 32)
    ...
    x = [torch.randn((2, 64, 208,272)),
         torch.randn((2, 128, 104,136)),
         torch.randn((2, 256, 52,68)),
         torch.randn((2, 512, 26, 34))]
