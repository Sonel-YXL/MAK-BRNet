...
class MakeBorderMap:
    ...
    def distance(self, xs, ys, point_1, point_2):
        '''
        compute the distance from point to a line  博客:核心方法：计算扩张后的边界框内所有点到该条边的距离
        ys: coordinates in the first axis
        xs: coordinates in the second axis
        point_1, point_2: (x, y), the end of the line  yxl:四个点中的相邻两个点
        '''
    ...
    def draw_border_map(self, polygon, canvas, mask):
        ...
        if xmin_valid - xmin >= width or ymin_valid - ymin >= height:  
            raise Exception("这里需要理解下代码")
            return  
    ...
    def __call__(self, data: dict) -> dict:
        ...
        ignore_tags = data['ann']['ignore_tags']  
        ...
        data['ann']['threshold_map'] = canvas
        data['ann']['threshold_mask'] = mask
        cv2.fillPoly(canvas, text_polys.astype(np.int32), 0.7)  
        return data
