...
def shrink_polygon_pyclipper(polygon, shrink_ratio):  
    ...
    distance = polygon_shape.area * (1 - np.power(shrink_ratio, 2)) / (polygon_shape.length+1e-05)  
    ...
    return shrinked  
class MakeShrinkMap():
    r'''  shrink_mask是获得的不参与识别的目标mask，shrink_map是经过内缩图的
    ...
    def __call__(self, data: dict) -> dict:
        ...
        data['ann']['ignore_tags'] = ignore_tags  
        data['ann']['shrink_map'] = gt  
        data['ann']['shrink_mask'] = mask
        return data
